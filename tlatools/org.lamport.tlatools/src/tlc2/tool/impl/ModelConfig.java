 ...
 * using our own collection implmentations like {@link Vect}.) TODO we're
 * storing a heterogeneous mishmash in the values of configTbl - sometimes a
 * Vect, sometimes a String, sometime that Vect has only String instances,
 * sometimes is has a String instance and Value subclasses, ... it would be nice
 * were the design cleaner.
 * 
...
public class ModelConfig implements ValueConstants, Serializable {
	...
	public final static String[] ALL_KEYWORDS = { Constant, Constants, Constraint, Constraints, ActionConstraint,
			ActionConstraints, Invariant, Invariants, Init, Next, View, Symmetry, Spec, Prop, Props, Alias, PostCondition,
			CheckDeadlock };
	 ...
	 * @param resolver       the name to stream resolver or <code>null</code> is the
		...
		try {
			...
			if (fis == null) {
				throw new ConfigFileException(EC.CFG_ERROR_READING_FILE,
						new String[] { this.configFileName, "File not found." });
			}
			...
			while (tt.kind != TLAplusParserConstants.EOF) {
				...
				if (tval.equals(Init)) {
					...
					tt = getNextToken(tmgr);
					if (tt.kind == TLAplusParserConstants.EOF) {
						throw new ConfigFileException(EC.CFG_MISSING_ID, new String[] { String.valueOf(loc), PostCondition });
					}
					String old = (String) this.configTbl.put(PostCondition, tt.image);
					if (old.length() != 0) {
						throw new ConfigFileException(EC.CFG_TWICE_KEYWORD, new String[] { String.valueOf(loc), PostCondition });
					}
					tt = getNextToken(tmgr);
					...
					while ((tt = getNextToken(tmgr)).kind != TLAplusParserConstants.EOF) {
						/*
						 * Exit this while loop if the next token is something like "CONSTANT" that
						 * starts a new section of the configuration file.
						 */
						...
						/*
						 * Token tt should be the first token in an expression of the form id <- ... or
						 * id = ... . In the current implementation, id is the token tt. The following
						 * code was modified on 30 July 2009 to allow id to be something like
						 * frob!bar!glitch, fixing Bug44.
						 */
						String lhs = tt.image;
						...
						if (tt.image.equals("<-")) {
							...
							if (tt.image.equals("[")) {
								...
								tt = getNextToken(tmgr, buf);
								if (tt.kind == TLAplusParserConstants.EOF) {
									throw new ConfigFileException(EC.CFG_EXPECT_ID,
											new String[] { String.valueOf(scs.getBeginLine()), "<-[" });
								}
								String modName = tt.image;
								tt = getNextToken(tmgr, buf);
								if (!tt.image.equals("]")) {
									throw new ConfigFileException(EC.CFG_EXPECTED_SYMBOL,
											new String[] { String.valueOf(scs.getBeginLine()), "]" });
								}
								tt = getNextToken(tmgr, buf);
								if (tt.kind == TLAplusParserConstants.EOF) {
									throw new ConfigFileException(EC.CFG_EXPECT_ID,
											new String[] { String.valueOf(scs.getBeginLine()), "<-[mod]" });
								}
							...
							if (!tt.image.equals("=")) {
								throw new ConfigFileException(EC.CFG_EXPECTED_SYMBOL,
										new String[] { String.valueOf(scs.getBeginLine()), "= or <-" });
							}
							tt = getNextToken(tmgr, buf);
							if (tt.image.equals("[")) {
					...
					if (tt.image.equals("TRUE")) {
						...
						throw new ConfigFileException(EC.CFG_EXPECTED_SYMBOL,
								new String[] { String.valueOf(scs.getBeginLine()), "TRUE or FALSE" });
					}
					if (previous != "undef") {
					...
					throw new ConfigFileException(EC.CFG_EXPECTED_SYMBOL,
							new String[] { String.valueOf(scs.getBeginLine()), "a keyword" });
				}
			}
	...
	private Value parseValue(Token tt, SimpleCharStream scs, TLAplusParserTokenManager tmgr, final StringBuffer buf)
		...
		throw new ConfigFileException(EC.CFG_EXPECTED_SYMBOL,
				new String[] { String.valueOf(scs.getBeginLine()), "a value" });
	}
	 ...
	 * strings). If one element, it has the form `["field->value"]`, which is a
	 * replacement, otherwise it has the form `["field", "value"]`, which is an
	 * assignment (which are the lines in a config file for the CONSTANT(s) section
	 * where you have `field = value`).
	 */
