 ...
 */
public class CCSimulationWorker extends IdThread implements INextStateFunctor {
	...
	// The base name of the file that this worker writes out generated traces to. If
	// it is null,
	// no trace files are generated.
	private final String traceFile;

	// A counter that tracks the number of generated states/traces. This counter may
	// be
	// shared among workers, so its count may be greater than the number of
	// states/traces
	...
	public CCSimulationWorker(int id, ITool tool, BlockingQueue<SimulationWorkerResult> resultQueue, long seed,
	...
	/**
	 * The main worker loop. Continually generates random traces until the trace
	 * count limit is reached or until the worker is interrupted.
	 * 
	 * We use the standard Java notion of thread "interruption" as a mechanism for
	 * halting/cancelling the execution of a worker thread. There's nothing
	 * particularly special about this. The Thread.interrupt() just sets a boolean
	 * flag that the running thread then periodically checks via calls to
	 * Thread.interrupted(). We could implement this manually but it's simpler to
	...
	public Object addElement(final TLCState s, final Action a, final TLCState t) {
		...
		if (!tool.isGoodState(t)) {
			...
			if (this.tool.getActions().length == 1) {
				...
				parameters = new String[] { a.getName().toString(), unassigned.size() > 1 ? "s are" : " is",
						unassigned.stream().map(n -> n.getName().toString()).collect(Collectors.joining(", ")) };
			}
		...
		try {
			for (idx = 0; idx < this.tool.getInvariants().length; idx++) {
				if (!tool.isValid(this.tool.getInvariants()[idx], t)) {
					...
					throw new SimulationWorkerError(EC.TLC_INVARIANT_VIOLATED_BEHAVIOR, new String[] { tool.getInvNames()[idx] },
							t, getTrace());
				}
			}
		} catch (final Exception e) {
	 ...
	 * 
	 * a) Pick one of the initial states. b) Randomly pick an action to generate the
	 * successor states (more than 1) to the current initial state. c) Check all of
	 * the generated successors for their validity. d) Randomly pick a generated
	 * successor and make it the new current state.
	 *
	 * Returns an Optional error representing the outcome of the trace generation.
	 * If the trace generation produced no error, returns Optional.empty().
	 *
	 */
	private Optional<SimulationWorkerError> simulateRandomTrace() throws Exception {
		...
		welfordM2AndMean.accumulateAndGet(Math.min(maxTraceDepth, curState.getLevel()), (acc, tl) -> {
			...
			// atomiclong because welfordM2AndMean is updated concurrently by multiple
			// workers).
